/**
 * @file benchmark_hnsw_arbitrary.cc
 * @author Chaoji Zuo (chaoji.zuo@rutgers.edu)
 * @brief Benchmark HNSW Baseline for Arbitrary Range Filtering
 * @date 2024-12-29
 *
...
int main(int argc, char **argv) {
  ...
  vector<int> searchef_para_range_list = {400};
  bool full_range = false;

  string indexk_str = "64";
  string ef_con_str = "400";
  string ef_max_str = "500";
  string ef_search_str = "400";
  string version = "HNSW_Baseline";
  ...
  cout << "HNSW Baseline Benchmark" << endl;
  cout << "index K:" << endl;
  ...
  for (unsigned index_k : index_k_list) {
    for (unsigned ef_max : ef_max_list) {
      for (unsigned ef_construction : ef_construction_list) {
        ...
        {
          cout << endl;
          cout << "Method: HNSW Baseline" << endl;
          ...
          BaseIndex::SearchInfo search_info(&data_wrapper, &i_params, "HNSW_Baseline",
                                            "benchmark");
          ...
          cout << "Total # of Neighbors: " << hnsw_index.index_info->nodes_amount
               << endl;
